...
def acquire(tid,lock_id,thread_info="", lock_info="",call_location=None):
	forrest.executeCommand(AcquireRelease.acquire,(Thread(tid,thread_info),Lock(lock_id,lock_info),call_location))

def release(tid,lock_id,thread_info="", lock_info="",call_location=None):
	forrest.executeCommand(AcquireRelease.release,(Thread(tid,thread_info),Lock(lock_id,lock_info),call_location))
...
class LockForrest:
	...
	def executeCommandonTree(self,thread,command,args=(),kwargs={}):
		"""executes the command with self.trees as argument
		command has to be callable """
		with self.mutex:
			tree = Utils.getTreeForThread(thread)
			if tree is not None:
